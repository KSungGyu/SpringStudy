...
select IFNULL(max(idx)+1, 1),'bit01','게시판연습','게시판연습','관리자',now(),0,IFNULL(max(boardGroup)+1,0),0, 0, 1 from tblBoard; 

select * from tblBoard;

insert into tblBoard
select IFNULL(max(idx)+1, 1),'bit02','게시판연습','게시판연습','김성규',now(),0,IFNULL(max(boardGroup)+1,0),0, 0, 1 from tblBoard; 

select * from tblBoard;

insert into tblBoard
select IFNULL(max(idx)+1, 1),'bit03','게시판연습','게시판연습','홍길동',now(),0,IFNULL(max(boardGroup)+1,0),0, 0, 1 from tblBoard; 
...
insert into tblMember(memID, memPwd, memName, memPhone)
values('bit02','bit02','김성규','010-2222-2222');
insert into tblMember(memID, memPwd, memName, memPhone)
